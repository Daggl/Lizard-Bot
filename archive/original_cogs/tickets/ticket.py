...
# Configuration: adjust these IDs or leave None to attempt automatic lookup
...
TICKET_CATEGORY_ID: Optional[int] = _CFG.get("TICKET_CATEGORY_ID", 1270347714931986485)
SUPPORT_ROLE_ID: Optional[int] = _CFG.get("SUPPORT_ROLE_ID", 1266608353421955153)
TICKET_LOG_CHANNEL_ID: Optional[int] = _CFG.get("TICKET_LOG_CHANNEL_ID", 1473793741197148250)
...
class TicketPanelView(discord.ui.View):
    """Persistent view used to create new tickets via a button."""

    def __init__(self, cog: "TicketCog") -> None:
        super().__init__(timeout=None)
        self.cog = cog

    @discord.ui.button(label="Create Ticket", style=discord.ButtonStyle.green, emoji="🎫", custom_id="ticket_panel:create")
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.defer(ephemeral=True)
        await self.cog._create_ticket_for_user(interaction.guild, interaction.user, interaction.channel)
        await interaction.followup.send("✅ Your support ticket has been created.", ephemeral=True)


class TicketControls(discord.ui.View):
    """View attached to each ticket channel with useful buttons."""

    def __init__(self, cog: "TicketCog") -> None:
        super().__init__(timeout=None)
        self.cog = cog

    @discord.ui.button(label="Close", style=discord.ButtonStyle.danger, emoji="🔒", custom_id="ticket_controls:close")
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.defer(ephemeral=True)
        await self.cog.close_ticket(interaction.channel, interaction.user)
        await interaction.followup.send("✅ Ticket closed.", ephemeral=True)

    @discord.ui.button(label="Claim", style=discord.ButtonStyle.secondary, emoji="👑", custom_id="ticket_controls:claim")
    ...
    @discord.ui.button(label="Transcript", style=discord.ButtonStyle.blurple, emoji="📄", custom_id="ticket_controls:transcript")
    async def transcript(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if path:
            ...
            await interaction.followup.send("❌ Failed to save transcript.", ephemeral=True)
...
class TicketCog(commands.Cog):
    ...
    Features:
    - Admin panel to post a ticket creation panel
    - Create a private ticket channel per user
    - Ticket controls (close, claim, transcript)
    - Admin commands to manage tickets and retrieve transcripts
    """

    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot
        # ensure new data dirs and migrate old files if present
        try:
            migrate_old_paths()
        except Exception:
            pass
        ensure_dirs()
        # SQLite DB for tickets
    ...
    async def _db_execute(self, query: str, params: tuple = (), fetch: bool = False):
        ...
        return await loop.run_in_executor(None, self._execute_sync, query, params, fetch)
    ...
    def _support_role(self, guild: discord.Guild) -> Optional[discord.Role]:
        ...
        for role in guild.roles:
            if role.permissions.manage_messages or role.name.lower() in ("support", "mod", "moderator"):
                return role
        return None
    ...
    async def _create_ticket_for_user(self, guild: discord.Guild, user: discord.Member, origin_channel: Optional[discord.TextChannel]) -> discord.TextChannel:
        """Create a ticket channel for a user with proper permissions."""
        ...
        # Log creation
        await self._log_action(guild, f"Ticket created: {channel.name} by {user} (id {user.id})")

        # Persist ticket to DB
        try:
            await self._db_execute(
                "INSERT OR REPLACE INTO tickets (channel_id, user_id, channel_name, created_at, status) VALUES (?, ?, ?, ?, ?)",
                (channel.id, user.id, channel.name, datetime.utcnow().isoformat(), "open"),
            )
    ...
    async def save_transcript(self, channel: discord.TextChannel) -> Optional[str]:
        """Save channel history to a text file and return the path."""
        try:
            ensure_dirs()
            ...
            await self._log_action(channel.guild, f"Transcript saved for {channel.name} -> {path}")
    ...
    async def close_ticket(self, channel: discord.TextChannel, by: discord.Member) -> None:
        """Close a ticket: save transcript and archive (delete) the channel."""
        await self.save_transcript(channel)
        await self._log_action(channel.guild, f"Ticket closed: {channel.name} by {by}")
        # Update DB closed state
        try:
            await self._db_execute(
    ...
    async def claim_ticket(self, channel: discord.TextChannel, by: discord.Member) -> None:
        """Mark a ticket as claimed by editing the topic and notifying the channel."""
    ...
    async def ticket_panel(self, ctx: commands.Context) -> None:
        """Post a ticket creation panel for users to open tickets."""
        embed = discord.Embed(
    ...
    async def ticket_command(self, ctx: commands.Context) -> None:
        """Create a ticket using command (fallback if panel is not used)."""
        await ctx.message.delete()
        await self._create_ticket_for_user(ctx.guild, ctx.author, ctx.channel)
        await ctx.author.send("Your ticket was created.")

    # Admin utilities: transcript retrieval and manual close
    @commands.command(name="transcript")
    @commands.has_permissions(administrator=True)
    async def transcript_cmd(self, ctx: commands.Context, channel: discord.TextChannel) -> None:
        """Save transcript of a ticket channel and attach the file."""
    ...
    @commands.has_permissions(administrator=True)
    async def close_ticket_cmd(self, ctx: commands.Context, channel: discord.TextChannel) -> None:
        """Close a ticket channel (admin only)."""
...
async def setup(bot: commands.Bot) -> None:
    ...
    # register persistent views if the bot restarts
